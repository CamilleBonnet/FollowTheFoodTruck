class Api::V1::ChoicesController < Api::V1::BaseController
  ...
  def update
    ...
    basket = Basket.where(user: current_registration.user.id, status: "pending", truck: meal.truck).last || Basket.create(user: current_registration.user.id, status: "pending", truck: meal.truck)
